...
#first_cover=""
	...
	new_cover=$(echo "$new_cover"|cut -c 32-)
	proper_prefix="https://i.scdn.co/image/"
	proper_cover_url="$proper_prefix$new_cover"
	
	if [[ $proper_cover_url != $old_cover ]]
	then
		old_cover="$proper_cover_url"
		wget -O ~/.config/conky/last_album_pic.png $proper_cover_url
		echo $proper_cover_url > ~/.config/conky/cover.txt
		#echo $old_cover
	fi
